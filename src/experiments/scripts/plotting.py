...
def update_plot():
    ...
    with open('../latency/data/timeDifferences.csv', 'r') as file:
        ...
        for row in reader:
            ...
            #second_values.append(row[1].strip().lower() == 'false')
    ...
    # Plot second value
    plt.plot(x_values, timeWithTelegrafSlow, label='Time difference between sent and received message with Telegraf - 1s', linestyle='-', color='magenta')
    plt.plot(x_values, timeWithTelegrafFast, label='Time difference between sent and received message with Telegraf - 0.001s', linestyle='-', color='green')
    ...
    #plt.plot(x_values, first_values, label='ActualSpeed', linestyle='--', marker='o', color='cyan')
    ...
    '''
    for x, y, is_optimized in zip(x_values, first_values, second_values):
        if is_optimized:
            plt.scatter(x, y, color='red', marker='x', zorder=2, label='Validation triggered' if 'Validation triggered' not in plt.gca().get_legend_handles_labels()[1] else "")
        if y == -0.0018689027240879064:
            plt.scatter(x, y, color='yellow', marker='o', zorder=3, label='False negative' if 'False negative' not in plt.gca().get_legend_handles_labels()[1] else "")
    '''
    # Add labels and title
    plt.xlabel('Execution time (s)')
    plt.ylabel('Message latency (s)')
    plt.title('Plot of latency test between send and received message - Comparing different Telegraf parameters with no Telegraf')
    ...
    plt.savefig('timeDiffsConclusion.png', dpi=300)  # Save as PNG file with high resolution
