...
def update_plot():
    ...
    with open('../SLAM/data/HectorStandingStill.csv', 'r') as file:
    ...
    plt.plot(x_values, second_values, label='Expected speed', linestyle='--', marker='o', color='red')

    # Highlight the points based on the third column
    for x, y, is_optimized in zip(x_values, second_values, third_values):
        if is_optimized:
            plt.scatter(x, y, color='orange', zorder=2, label='Expected speed (Optimized)' if 'Expected speed (Optimized)' not in plt.gca().get_legend_handles_labels()[1] else "")

    # Add labels and title
    plt.xlabel('Time (s)')
    plt.ylabel('Speed (m/s)')
    plt.title('Plot of ActualSpeed and ExpectedSpeed using hector SLAM (Smoothed)')
    plt.legend()
    ...
    plt.savefig('hectorStill.png', dpi=300)  # Save as PNG file with high resolution
